...
public final class NetworkInterceptor implements Interceptor {
    ...
    @Override public Response intercept(Chain chain) throws IOException {
        ...
        if (url.contains(RetrofitAPI.KAKAO_API_URL)) {
            requestBuilder.addHeader("Authorization", "KakaoAK cb2d2132353cd5f046489bfbc610da01");
        }
        ...
        try {
            if (!parser.parse(result).isJsonObject()) {
                ...
                jsonTemp = parser.parse(result).getAsJsonObject();
            }
        } catch (Exception e) {
            e.printStackTrace();
            jsonTemp = new JsonObject();
        ...
        if (jsonObject.has("ACCESS-TYPE")) {
            if (jsonObject.get("ACCESS-TYPE").getAsString().equals("Re-Try")) {
                intercept(chain);
                return null;
            } else {
                return response;
